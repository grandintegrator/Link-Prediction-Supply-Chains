...
from model.dgl.layers import BaseRGCN
from ingestion.utils import *
# from kg_utils.utils_raw import *
# import utils
...
def main(args):
    ...
    from kg_data_loader import SupplyKnowledgeGraphDataset
    loader = SupplyKnowledgeGraphDataset()

    data_sp = loader[0]

    num_nodes_sp = data_sp.num_nodes()
    train_data_sp = np.asarray(loader.train_graph)
    valid_data_sp = np.asarray(loader.valid_graph)
    test_data_sp = np.asarray(loader.valid_graph)
    num_rels_sp = loader.num_rels

    train_data = train_data_sp
    valid_data = valid_data_sp
    ...
    # num_noes = train_data.max()
    adj_list, degrees = get_adj_and_degrees(num_nodes_sp, train_data_sp)
    ...
    while True:
        ...
        g, node_id, edge_type, node_norm, data, labels = \
            generate_sampled_graph_and_labels(
                train_data_sp, args.graph_batch_size, args.graph_split_size,
